...
def SimpleLoader(data_path,
    ...
    mean_a = np.mean(np.mean(cells, axis=0),axis=0) # [-inf -2.7222664e+01  1.6344410e+00  1.0989834e+03 2.6169461e-01]
    min_a = np.min(np.min(cells, axis=0),axis=0) # [-inf -2700. -2700.     0.     0.]
    max_a = np.max(np.max(cells, axis=0),axis=0) # [-7.043041e-01  2.700000e+03  2.600000e+03  5.085100e+03  1.000000e+00]

    interval_array = np.linspace(min_a[2], max_a[2], 55) # interval created in (-2700,2600)

    if(clusters.shape[1]==2):

            print('cluster_s cdim hanging to 56 dim')
            clusters_new = []

            for i in tqdm.tqdm(range(clusters.shape[0])): # for each event

                # dividing each event (200 partciles) into 54 intervals.
                part_event = cells[i]
                z_values = part_event[:, 2]

                # Initialize a dictionary to store the sum of E values for each interval
                interval_sums = {i: 0 for i in range(len(interval) - 1)}

                # Iterate over the data list [200,5]
                for entry in part_event:
                    x, y, z, E, unk = entry
                    # Check which interval the z value falls into
                    for i in range(len(interval) - 1): # [54]
                        if interval[i] <= z < interval[i + 1]:
                            interval_sums[i] += E
                            break  # Break the loop once we find the interval

                # Extract the summed E values as a list
                summed_E_values = [interval_sums[i] for i in range(len(interval) - 1)]
                summed_E_values = np.array(summed_E_values, dtype="float32")
                sums_of_E = summed_E_values.reshape(-1,1)
                ...
                z_intervals = [(interval[i], interval[i+1]) for i in range(len(interval) - 1)]

                for ind,val in enumerate(part_event): # [200,5 ]
                    x, y, z, E, unk = val
                    for p,interval_0 in enumerate(z_intervals): # [54]
                        if (interval_0[0] <= z < interval_0[1]) and (summed_E_values[p]>1e-10):
                            new_E = E / summed_E_values[p] # summed_E_values[p] = energy of the layer.
                            cells[i][ind][3] = new_E
                            break  # Once we find the interval, we can stop searching
                        else:
                            cells[i][ind][3] = E 
            ...
            print('1',cells.shape)
            print('2',np.mean(np.mean(cells, axis=0),axis=0)) # [-inf -2.7222664e+01  1.6344410e+00  1.0989834e+03 2.6169461e-01]
            print('3',np.min(np.min(cells, axis=0),axis=0)) # [-inf -2700. -2700.     0.     0.]
            print('4',np.max(np.max(cells, axis=0),axis=0)) # [-7.043041e-01  2.700000e+03  2.600000e+03  5.085100e+03  1.000000e+00]


            #print(np.mean(cells[1]))
            print(len(clusters_new))
            print(clusters_new[1].shape)
            print('clusters_prev',clusters.shape)
            clusters = np.array(clusters_new)

            #clusters = np.vstack(clusters_new)
...
def DataLoader(data_path,labels,
    ...
    def _preprocessing(cells,clusters,save_json=False):
        ...
        interval = np.linspace(min_a[2], max_a[2], 55) # interval created in (-2700,2600)
        # If clusters is of 2 dim then only make it 56 dim.
        # While testing, if the save_dic is already having cluster of 56 dim, then don't enter.
        if(clusters.shape[1]==2):
        ...
        # save the mean and std of celss and clusters (54 dim)
        if save_json:
        ...
        print('mean', np.mean( np.mean(cells, axis=0),axis = 0)) # mean [ 1.0650898 17.038385 ]
        print('min', np.min( np.min(cells, axis=0),axis = 0)) # min [9.4318224e-05 1.5567589e+01]
        print('max',np.max( np.max(cells, axis=0),axis = 0)) # ax [ 2.1198688 18.432405 ]
    ...
    interval_array = np.linspace(min_a[2], max_a[2], 55) # # Replace with self.num_z_layers +1 ,interval created in (-2700,2600)
